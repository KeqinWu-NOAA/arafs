...
if [ $input_type = "grib2" ]; then
  if [ $ictype = gfsgrib2ab_0p25 ]; then
    ...
    ${RLN} ${INIDIR}/${grib2_file_input_grid} ./
  fi
  INPDIR="./"
else
  if [ ${ENSDA} = YES ]; then
   ${RLN} ${INIDIR}/${atm_files_input_grid} ./
   ${RLN} ${INIDIR}/${sfc_files_input_grid} ./
   INPDIR="./"
  elif [ ${FGAT_MODEL} = gdas ]; then
   ${RLN} ${INIDIR}/${atm_files_input_grid} ./
   ${RLN} ${INIDIR}/${sfc_files_input_grid} ./
...
if [ $gtype = uniform ] || [ $gtype = stretch ] || [ $gtype = nest ]; then
  ...
  if [ $gtype = nest ]; then
    ${RLN} $FIXDIR/$CASE/${CASE}_coarse_mosaic.nc $FIXDIR/$CASE/${CASE}_mosaic.nc
  fi
...
# Set the links to use the 4 halo grid and orog files, which are necessary for creating the boundary data
  ${RLN} $FIXDIR/$CASE/${CASE}_grid.tile7.halo4.nc $FIXDIR/$CASE/${CASE}_grid.tile7.nc
  ${RLN} $FIXDIR/$CASE/${CASE}_oro_data.tile7.halo4.nc $FIXDIR/$CASE/${CASE}_oro_data.tile7.nc
  ${RLN} $FIXDIR/$CASE/fix_sfc/${CASE}.vegetation_greenness.tile7.halo4.nc $FIXDIR/$CASE/${CASE}.vegetation_greenness.tile7.nc
  ${RLN} $FIXDIR/$CASE/fix_sfc/${CASE}.soil_type.tile7.halo4.nc $FIXDIR/$CASE/${CASE}.soil_type.tile7.nc
  ${RLN} $FIXDIR/$CASE/fix_sfc/${CASE}.slope_type.tile7.halo4.nc $FIXDIR/$CASE/${CASE}.slope_type.tile7.nc
  ${RLN} $FIXDIR/$CASE/fix_sfc/${CASE}.substrate_temperature.tile7.halo4.nc $FIXDIR/$CASE/${CASE}.substrate_temperature.tile7.nc
  ${RLN} $FIXDIR/$CASE/fix_sfc/${CASE}.facsf.tile7.halo4.nc $FIXDIR/$CASE/${CASE}.facsf.tile7.nc
  ${RLN} $FIXDIR/$CASE/fix_sfc/${CASE}.maximum_snow_albedo.tile7.halo4.nc $FIXDIR/$CASE/${CASE}.maximum_snow_albedo.tile7.nc
  ${RLN} $FIXDIR/$CASE/fix_sfc/${CASE}.snowfree_albedo.tile7.halo4.nc $FIXDIR/$CASE/${CASE}.snowfree_albedo.tile7.nc
  ${RLN} $FIXDIR/$CASE/fix_sfc/${CASE}.vegetation_type.tile7.halo4.nc $FIXDIR/$CASE/${CASE}.vegetation_type.tile7.nc
...
${APRUNC} ./hafs_utils_chgres_cube.x
...
inest=$(($itile + 2 - $stile))
${RLN} $FIXDIR/$CASE/fix_sfc/${CASE}*.nc $FIXDIR/$CASE/.
${RLN} $FIXDIR/$CASE/${CASE}_nested0${inest}_mosaic.nc $FIXDIR/$CASE/${CASE}_mosaic.nc
