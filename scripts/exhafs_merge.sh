...
if [[ ( $ENS = "16" ) || ( $ENS = "17" ) || ( $ENS = "18" ) || ( $ENS = "26" ) || ( $ENS = "27" ) || ( $ENS = "28" ) ]]; then
  export YMDHx=${YMDHm6}
  echo "$YMDHx"
elif [[ ( $ENS = "19" ) || ( $ENS = "20" ) || ( $ENS = "29" ) || ( $ENS = "30" ) ]]; then
  export YMDHx=${YMDHm12}
  echo "$YMDHx"
else
  export YMDHx=$YMDH
  echo "$YMDHx"
...
#    export RESTARTsrc=/scratch1/NCEPDEV/hwrf/save/Jiayi.Peng/VItest/LinZhu/${num}l.${YMDHx}.RESTART
    export RESTARTsrc=${DATA}/${num}l.${YMDHx}.RESTART
    if [ ! -d ${RESTARTsrc} ]; then
      mkdir -p ${RESTARTsrc}
    fi    

    if [ -d ${INPUTsrc}/${num}l.${YMDH}.RESTART ]; then
######################## current cycle with the storm exist	    
    if [ -d ${INPUTsrc}/${num}l.${YMDHx}.RESTART ]; then
    cp ${INPUTsrc}/${num}l.${YMDHx}.RESTART/${ymd}.${hh}0000.*.nest02.tile2.nc ${RESTARTsrc}/.
    cp ${INPUTsrc}/${num}l.${YMDHx}.RESTART/grid_mspec.nest02_${yr}_${mn}_${dy}_${hh}.tile2.nc ${RESTARTsrc}/.
...
#--------
      ...
      rm -f tmpvit tmpvit_new	    
      echo "${INPUTsrc}/${num}l.${YMDHx}.RESTART is missing, no storm to be merged"
    fi   
####################   current cycle with the storm exist 
    else
      rm -f tmpvit tmpvit_new
      echo "${INPUTsrc}/${num}l.${YMDH}.RESTART is missing, no storm to be merged"	    
    fi
    else
