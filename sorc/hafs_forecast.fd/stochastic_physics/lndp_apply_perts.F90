...
! the parameters are updated by the model.
    ...
    subroutine lndp_apply_perts(blksz, lsm, lsm_noah, lsm_ruc, lsm_noahmp, iopt_dveg, & 
        ...
        real(kind=kind_phys), intent(inout), optional :: zorlla(:,:)
        real(kind=kind_phys), intent(inout), optional :: zorlwa(:,:)
             ...
             do v = 1,n_var_lndp
                ...
                case('zol')  ! land roughness length
                    if (do_pert_param) then
                         ...
                         min_bound=0.
                         max_bound=300.
!                         print *, "lndp_prt_list(v),tfactor_param=", lndp_prt_list(v), tfactor_param
                         pert = sfc_wts(nb,i,v)*lndp_prt_list(v)
!                         pert = pert*tfactor_param
                         if(zorlla(nb,i) .lt. 0.0) zorlla(nb,i)=0.0
                         if(zorlwa(nb,i) .lt. 0.0) zorlwa(nb,i)=0.0
                         if(zorlla(nb,i) .eq. 9.969209968386869E+036) zorlla(nb,i)=0.0
                         if(zorlwa(nb,i) .eq. 9.969209968386869E+036) zorlwa(nb,i)=0.0

                         call apply_pert ('zol',pert,print_flag, zorlla(nb,i), ierr,p,min_bound, max_bound)

!                         if(zorlwa(nb,i) .gt. 0.0) then
!                         print *, "pert=", pert, "zorlwa(nb,i)_before=", zorlwa(nb,i)
!                         endif
                         call apply_pert ('zol',pert,print_flag, zorlwa(nb,i), ierr,p,min_bound, max_bound)
!                         print *, "zorlwa(nb,i)_after=", zorlwa(nb,i)
                     endif
                case default
                    print*, &
  ...
  subroutine apply_pert(vname,pert,print_flag, state,ierr,p,vmin, vmax)
   ...
   ! intent in
    ...
    real(kind=kind_phys) :: z
       ...
       ! apply perturbation
!       print *, "pert, state_before=", pert, state
!       if (present(p) ) then
...
!       endif
!J.Peng  2024/05/01
! sfc-perts: add Z0 perturbations over ocean
!      if (pert /= zero ) then
       state = state * (10.0**pert)
!      endif
!       print *, "state_after=", state
