module stochastic_physics_wrapper_mod
  ...
  real(kind=kind_phys), dimension(:,:),   allocatable, save :: zorlla
  real(kind=kind_phys), dimension(:,:),   allocatable, save :: zorlwa
    ...
    initalize_stochastic_physics: if (.not. is_initialized) then
      ...
      if ( GFS_Control%lndp_type == 2 ) then
          ...
          do v = 1,GFS_Control%n_var_lndp
            ...
            case('zol') 
              allocate(zorlla (1:nblks, maxblk))
              allocate(zorlwa (1:nblks, maxblk))
            endselect 
          enddo 
      endif
         ...
         if (GFS_Control%lndp_type == 2) then ! save wts, and apply lndp scheme
             ...
             do nb=1,nblks
                do v = 1,GFS_Control%n_var_lndp
                  ...
                  case('zol')
                      zorlla(nb,1:GFS_Control%blksz(nb))  = GFS_Data(nb)%Sfcprop%zorll(1:GFS_Control%blksz(nb))
                      zorlwa(nb,1:GFS_Control%blksz(nb))  = GFS_Data(nb)%Sfcprop%zorlw(1:GFS_Control%blksz(nb))
!                      if(nb .eq. 1) then
!                        print *, "zorlla(nb,1)=", zorlla(nb,1), "zorlwa(nb,1)=", zorlwa(nb,1)
!                      endif       
                  endselect
              enddo
             enddo
...
!                   print *, "GFS_Control%kdt,GFS_Control%nscyc=", GFS_Control%kdt, GFS_Control%nscyc
!                   print *, "param_update_flag0 =", param_update_flag
                 endif
             endif
             ...
             ! call once at start of the forecast.
                    param_update_flag = .true.
!                    print *, "param_update_flag1 =", param_update_flag
             endif
             if(GFS_Control%kdt .gt. 4) then 
!             print *, "lndp_apply_perts_before, zorlla(1,1)=", zorlla(1,1), "zorlwa(1,1)=", zorlwa(1,1)         
             call lndp_apply_perts(GFS_Control%blksz, GFS_Control%lsm, GFS_Control%lsm_noah, GFS_Control%lsm_ruc,             &
                               ...
                               smc, slc, stc, vfrac, alnsf, alnwf, snoalb, semis, zorlla, zorlwa, ierr)
!             print *, "lndp_apply_perts_after, zorlla(1,1)=", zorlla(1,1), "zorlwa(1,1)=", zorlwa(1,1)           
             endif
             if (ierr/=0)  then
             ...
             do nb=1,nblks
                do v = 1,GFS_Control%n_var_lndp
                  ...
                  case('zol')
!                        GFS_Data(nb)%Sfcprop%zorll(1:GFS_Control%blksz(nb))  = zorlla(nb,1:GFS_Control%blksz(nb))
                        GFS_Data(nb)%Sfcprop%zorlw(1:GFS_Control%blksz(nb))  = zorlwa(nb,1:GFS_Control%blksz(nb))
                  end select   
                enddo 
            enddo
         endif ! lndp block
  ...
  if (GFS_Control%do_sppt .OR. GFS_Control%do_shum .OR. GFS_Control%do_skeb .OR. (GFS_Control%lndp_type > 0) .OR. GFS_Control%do_spp) then
      ...
      if (GFS_Control%lndp_type == 2) then
          ...
          if (allocated(zorlla))  deallocate(zorlla)
          if (allocated(zorlwa))  deallocate(zorlwa)
      endif
